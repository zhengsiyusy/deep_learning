# / -*-codeing = utf-8  -*-
# TIME : 2022/9/22 16:39
# File : shichatu_cal  计算视差图
import numpy as np
import cv2
import stereo
...
if __name__ == "__main__":
    ...
    P1 = 600
    P2 = 2400
    imgL = cv2.imread(r'D:\healthy_QR_code_recongize\double_cam\SaveImage\left\left_0.jpg')
    imgR = cv2.imread(r'D:\healthy_QR_code_recongize\double_cam\SaveImage\right\right_0.jpg')
    imgL = cv2.resize(imgL, dsize=None, fx=0.2, fy=0.2)
    imgR = cv2.resize(imgR, dsize=None, fx=0.2, fy=0.2)

    cv2.namedWindow('disparity')
    cv2.createTrackbar('speckleRange', 'disparity', speckleRange, 50, update)
    cv2.createTrackbar('window_size', 'disparity', window_size, 21, update)  # 21
